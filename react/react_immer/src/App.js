...
*/
  ...
  // useCallback을 사용하지 않으면 리랜더링 될 때마다
  // 함수가 다시 만들어집니다.
  /*
  const onSubmit = useCallback(
    (e) => {
      ...
      // a 태그를 이용한 이동이나 form의 submit이나 reset 이벤트는 화면 전체를 새로 생성함.
      // 이전에 가지고 있던 내용을 모두 삭제합니다.
      // react, vue, angular는 SPA Framework라서 화면 전체를 다시 랜더링하면 기본틀이 무너집니다.
      // 화면에 출력된 내용과 가상의 DOM을 비교해서 변경된 부분만 리랜더링
        ...
        <ul>
          {data.array.map((info) => (
            <li key={info.id} onClick={() => onRemove(info.id)}>
              아이디 (이름): {info.username} ({info.name})
            </li>
          ))}
        </ul>
      </div>
